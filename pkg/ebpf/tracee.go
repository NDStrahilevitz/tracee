...
func New(cfg Config) (*Tracee, error) {
	...
	for filterScope := range t.config.FilterScopes.Map() {
		for e := range filterScope.EventsToTrace {
			...
			t.events[e] = eventConfig{submit: submit, emit: true}
		}
	}
...
func (t *Tracee) populateBPFMaps() error {
	// Prepare events scope map
	eventsScopeMap, err := t.bpfModule.GetMap("event_scopes_map")
	if err != nil {
		return err
	}
	for id, ecfg := range t.events {
		submit := ecfg.submit
		fmt.Printf("submit %d: %b\n", id, submit)
		eventsScopeMap.Update(unsafe.Pointer(&id), unsafe.Pointer(&submit))
	}
