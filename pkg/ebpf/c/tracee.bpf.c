...
BPF_PROG_ARRAY(sys_enter_submit_tail, 1);
BPF_PROG_ARRAY(sys_exit_submit_tail, 1);
...
// we pass a pointer to a pointer here so we can move from a NULL pointer
static __always_inline int init_task_info(task_info_t **task_info, u32 key, void *dump)
{
    ...
    // dump argument.
    int exist = bpf_map_update_elem(&task_info_map, &key, dump, BPF_NOEXIST);
...
static __always_inline int init_event_data(event_data_t *data, void *ctx)
{
    ...
    // init task_info with garbage data from submit buffer
    int result = init_task_info(&data->task_info, data->context.task.host_tid, data->submit_p);
...
{
    struct task_struct *task = (struct task_struct *) bpf_get_current_task();
    u64 task_id = bpf_get_current_pid_tgid(); // get the tgid only
    ...
    init_task_info(&task_info, task_id, config);
    if (task_info == NULL) {
        return 0;
    }
...
int tracepoint__raw_syscalls__sys_exit(struct bpf_raw_tracepoint_args *ctx)
{
    ...
    // after sys_enter task_info should always exist
    task_info_t *task_info = (task_info_t *) bpf_map_lookup_elem(&task_info_map, &task_id);
    if (task_info == NULL) {
        return 0;
    }
    int zero = 0;
...
int sys_exit_submit(struct bpf_raw_tracepoint_args *ctx)
{
    ...
    if (should_submit(sys->id, data.config) && (id != SYSCALL_EXECVE && id != SYSCALL_EXECVEAT) ||
