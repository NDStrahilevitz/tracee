...
type stdioOverSocket struct {
	...
	pidFDAddressMap map[int]map[int]trace.SockAddr
}

func (sig *stdioOverSocket) Init(cb detect.SignatureHandler) error {
	sig.cb = cb
	sig.pidFDAddressMap = make(map[int]map[int]trace.SockAddr)
...
func (sig *stdioOverSocket) OnEvent(event protocol.Event) error {
	...
	case "security_socket_connect":
		...
		err = isSocketOverStdio(sig, event, sockAddr, sockfd)
		if err != nil {
			return err
		}
		...
		if !pidExists {
			sig.pidFDAddressMap[pid] = make(map[int]trace.SockAddr)
		}

		if sockAddr.Address() != "" && sockAddr.Port() != 0 {
			sig.pidFDAddressMap[pid][sockfd] = sockAddr
		}
...
func isSocketDuplicatedIntoStdio(sig *stdioOverSocket, event protocol.Event, pidSocketMap map[int]trace.SockAddr, srcFd int, dstFd int) error {
...
func isSocketOverStdio(sig *stdioOverSocket, event protocol.Event, address trace.SockAddr, fd int) error {
